...
class VGG(nn.Module):
    ...
    def norm(self, image):
        ...
        mean = torch.tensor([0.1661, 0.1661, 0.1661]).to(DEV)
        std = torch.tensor([0.4235, 0.4235, 0.4235]).to(DEV)
    ...
    def feature_loss(self, target_feature, content1_feature, content2_feature):
        ...
        return content_loss
    # 定义特征保真度损失函数
    def vgg_loss(self, fused, ir, vs):
        fused_features = self.get_features(fused)
        ir_features = self.get_features(ir)
        vs_features = self.get_features(vs)
        loss = 0
        for layer in ['conv2_1', 'conv3_1', 'conv4_2']:
            max_features = torch.max(ir_features[layer], vs_features[layer])
            loss += torch.mean((fused_features[layer] - max_features) ** 2)
